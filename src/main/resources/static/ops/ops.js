    ...
    const recon = {
        async load() {
            ...
            try {
                ...
                tbody.innerHTML = rows.map(r => `<tr><td>#${r.id}</td><td>${badge(r.type)}</td><td><a href="#" onclick="ops.recon.account('${esc(r.accountId)}');return false" class="ops-mono">${esc(r.accountId)}</a></td><td>${r.cardId ? `<a href="#" onclick="ops.card360.open(${r.cardId});return false">#${r.cardId}</a>` : '—'}</td><td class="ops-mono">${esc(r.approvalCode)}</td><td>${money(r.cmsAmount)}</td><td>${money(r.coreAmount)}</td><td class="ops-muted" style="min-width:260px;white-space:normal;font-size:11px">${esc(r.detail)}</td><td class="ops-mono">${dt(r.lastSeenAt)}</td><td>${badge(r.status)} ${r.resolution ? '<div class="ops-muted">' + esc(r.resolution) + '</div>' : ''}</td><td>${r.status === 'OPEN' ? `<button class="btn btn-emerald" style="padding:0.3rem 0.6rem;font-size:0.72rem" onclick="ops.recon.resolve(${r.id})">Resolver</button>` : ''}</td></tr>`).join('');
